...
def run() :
    fileList = [] # 파일 주소 리스트 
    filenames = os.listdir("./runs/detect/exp/crops")
    for filename in filenames:
            fileList.append("./runs/detect/exp/crops/" + filename)
    ...
    def preprocessing(image , num):      
        ...
        # 추출된 전경 이미지 저장 
        cv2.imwrite("foreground/foreground" + str(num) + ".png", result)
    ...
    def kmeans_color_to_hsv(foregroundList, clusterno):
        imgSum = [] 
        for img in foregroundList:   
    ...
    for i in range(len(imageList)):
        preprocessing(imageList[i] , i)
        foregroundList.append(cv2.imread("foreground/foreground" + str(i) + ".png"))
        foregroundList[i] = cv2.cvtColor(foregroundList[i], cv2.COLOR_BGR2RGB)
    ...
    clusterno = 5
    final_hsvlabels = kmeans_color_to_hsv(foregroundList, clusterno)
    ...
    result = df_origin[df_origin['key'] == closest_color]
    closest_tone = result.iloc[0,0]
    color1_h,color1_s,color1_v = result.iloc[0,[2,3,4]]
    color2_h,color2_s,color2_v = result.iloc[1,[2,3,4]]
    color3_h,color3_s,color3_v = result.iloc[2,[2,3,4]]
    
    print(result)
    return closest_tone,closest_color
#,color1_h,color1_s,color1_v,color2_h,color2_s,color2_v,color3_h,color3_s,color3_v
